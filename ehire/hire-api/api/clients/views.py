...
class ClientViewSet(GenericViewSet):
    ...
    def org_update(self,request):
        ...
        try:
            ...
            data=request.data
            id=data["id"]
            serializer=self.get_serializer(self.service.update_client(id),data=request.data)
    ...
    def org_get(self, request):
        ...
        try:
            id=request.GET["id"]
            serializer=self.get_serializer(self.service.get_client(id))
            return Response(serializer.data,status.HTTP_200_OK)
        except Exception as e:
            return Response({"status": "Not Found"}, status.HTTP_404_NOT_FOUND)
...
class JobViewSet(GenericViewSet):
    ...
    def job_get(self, request):
        ...
        try:
            id = request.GET["id"]
            serializer=self.get_serializer(self.service.get_job(id))
            return Response(serializer.data,status.HTTP_200_OK)
        except Exception as e:
            return Response({"status": "Not Found"}, status.HTTP_404_NOT_FOUND)
    ...
    def job_update(self,request):
        ...
        try:
            ...
            print(data)
            job_id=data["id"]
            serializer=self.get_serializer(self.service.update_job(id),data=request.data)
            if not serializer.is_valid():
                print(serializer.errors)
                raise ParseException(BAD_REQUEST,serializer.errors)
            else:
